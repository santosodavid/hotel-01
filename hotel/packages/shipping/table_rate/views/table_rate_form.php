<div class="row">
	<div class="span12">
		<select name="enabled" class="span3">
			<option value="1"
			<?php echo((bool)$enabled)?' selected="selected"':'';?>>
		...
		<div class="control-group">
			<div class="controls">
				<div class="input-append">
					<input type="text" id="add_name_input" class="span3"
						placeholder="<?php echo lang('table_name');?>" />
					<button class="btn" type="button" onclick='add_new_table()'>
...
<div id="kill_on_save">
	<div id="table_form_template" style="display: none">
		...
		<li class="active" id="table_var_count">
			...
			onclick="toggle_table(var_count); return false;">var_name</a>
									...
									<div class="row">
										<div class="span9">
											<div class="form-inline pull-right" style="margin: 10px 0px;">
												<input type="input" class="span1"
													placeholder="<?php echo lang('from');?>"
													id="from_field_var_count" /> <input type="input"
													class="span1" placeholder="<?php echo lang('rate');?>"
													id="rate_field_var_count" />
												<button type="button" class="btn"
													onclick="add_rate(var_count, $('#from_field_var_count').val(),$('#rate_field_var_count').val()); $('#from_field_var_count').val('');$('#rate_field_var_count').val('')">
													<i class="icon-plus-sign"></i>
													<?php echo lang('add_rate');?>
												</button>
											</div>
										</div>
									</div>
	...
	<table style="display: none;">
		<tbody id="rate_form_template">
			<tr id="rate_var_count" class="form-inline">
				<td><a href="#" class="handle btn"><i class="icon-align-justify"></i>
				</a></td>
				<td><?php echo lang('from');?>
				</td>
				<td><input id="from_line_var_count" type="input"
					name="rate[var_table_count][rates][var_count][from]" class="span1" />
				</td>
				<td><?php echo lang('rate');?>
				</td>
				<td><input id="rate_line_var_count" type="input"
					name="rate[var_table_count][rates][var_count][rate]" class="span1" />
				</td>
				<td><span class="pull-right">
						<button class="btn btn-danger" type="button"
							onclick="delete_rate(var_count)">
							<i class="icon-trash icon-white"></i>
						</button>
				</span>
				</td>
