...
$(function() {
	...
	$('.home .datepicker_from').datepicker({
		...
		onSelect : function(date) {
			update_total_price();
			$(".datepicker_to").datepicker("option", "minDate", date);
		}
	});
	$('.home .datepicker_to').datepicker({
		...
		onSelect : function(date) {
			update_total_price();
			$(".datepicker_from").datepicker("option", "maxDate", date);
		}
	});
	...
	$('.book-start .datepicker_from').datepicker(
			{
				...
				onSelect : function(date) {
					update_total_price();
					$('.book-start .datepicker_from').datepicker("option",
	...
	$('.global-datepicker-from').datepicker(
			{
				...
				onClose : function(selectedDate) {
					$('.global-datepicker-from').datepicker("option",
							"minDate", selectedDate);
				}
			});
	...
	$('.global-datepicker-to').datepicker(
			{
				...
				minDate : 1,
				onClose : function(selectedDate) {
					$('.global-datepicker-to').datepicker("option", "maxDate",
							selectedDate);
				}
			});
...
$(function() {
	 ...
	 * $('.room_selector').position().top+50; var position_left =
	 * $('.book-now').position().left; $('#promotion_box').hide();
	 * $('#promotion_box').css({'left' : position_left+250, 'top' :
	 * position_top});
	 * 
	 * 
	 * /*$(document).mousemove(function(e){ if(mouseLastYPos){
	 * 
	 * if (mouseLastYPos > $('.room_selector').position().top){
	 * $('#promotion_box').show(); } else { $('#promotion_box').hide(); } }
	 * mouseLastYPos = e.pageY; });
	 */
	// }
	/*
	 * $(".home").hover( function () { var position_left =
	 * $('.book-now').position().left; $('#promotion_box').animate({ left:
	 * position_left+250 }, 1000, function() { // Animation complete. }); },
	 * function () { $('#promotion_box').css({'left' :
	 * $(window).width()+$('#promotion_box').width()}); } );
	 */
...
function update_extras_price() {
	...
	$('#extras_price').html("Rp " + String.format("{0:c}", extras_price));
...
function update_total_price() {
	...
	/*
	 * if (total_price < 100) { $('#total_price').css('letterSpacing', '2px'); }
	 */
	if (total_price >= 10000000) {
		$('#total_price').css('letterSpacing', '-1px');
	}
...
function initialize_map() {
	...
	var image = new google.maps.MarkerImage('css/images/icon_marker.png',
			new google.maps.Size(42, 62), new google.maps.Point(0, 0),
			new google.maps.Point(21, 62));

	var shape = {
		coord : [ 27, 0, 29, 1, 31, 2, 33, 3, 34, 4, 35, 5, 36, 6, 37, 7, 38,
				8, 38, 9, 39, 10, 39, 11, 40, 12, 40, 13, 40, 14, 41, 15, 41,
				16, 41, 17, 41, 18, 41, 19, 41, 20, 41, 21, 41, 22, 41, 23, 41,
				24, 41, 25, 41, 26, 40, 27, 40, 28, 40, 29, 40, 30, 39, 31, 39,
				32, 39, 33, 38, 34, 38, 35, 37, 36, 37, 37, 37, 38, 36, 39, 36,
				40, 35, 41, 35, 42, 34, 43, 34, 44, 33, 45, 33, 46, 32, 47, 31,
				48, 31, 49, 30, 50, 30, 51, 29, 52, 28, 53, 28, 54, 27, 55, 26,
				56, 26, 57, 25, 58, 24, 59, 23, 60, 22, 61, 20, 61, 18, 60, 17,
				59, 17, 58, 16, 57, 15, 56, 14, 55, 14, 54, 13, 53, 12, 52, 12,
				51, 11, 50, 11, 49, 10, 48, 9, 47, 9, 46, 8, 45, 8, 44, 7, 43,
				7, 42, 6, 41, 6, 40, 5, 39, 5, 38, 4, 37, 4, 36, 4, 35, 3, 34,
				3, 33, 2, 32, 2, 31, 2, 30, 2, 29, 1, 28, 1, 27, 1, 26, 1, 25,
				1, 24, 0, 23, 0, 22, 0, 21, 0, 20, 0, 19, 0, 18, 0, 17, 1, 16,
				1, 15, 1, 14, 1, 13, 2, 12, 2, 11, 3, 10, 3, 9, 4, 8, 5, 7, 6,
				6, 7, 5, 8, 4, 9, 3, 10, 2, 12, 1, 15, 0, 27, 0 ],
		type : 'poly'
	};
